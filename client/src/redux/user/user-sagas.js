...
export function* signUpWithEmail({ payload }) {
    try {
        const { data } = yield axios.post('api/signup', {
            ...payload
        });
        if (data.status == 'success') {
            yield put(signUpUserSuccess({ id: data.user._id, ...data.user }));
        }
    } catch (error) {
        yield put(emailSignInFailuer(error))
    }
...
export function* onSignUpStart() {
    yield takeLatest(userActionType.SIGN_UP_START, signUpWithEmail)
}
//2.5
